...
# Install Adapter {{{
COPY --chown=1000:1000 . /opt/fmc_repository/OpenMSA_Workflow_Kibana
RUN install_repo_deps.sh /opt/fmc_repository/OpenMSA_Workflow_Kibana/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/OpenMSA_Workflow_Kibana/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
